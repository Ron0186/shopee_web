<template>
  <div>
    ...
    <div>
      <label>選擇超商門市：</label>
      <button @click="openStorePicker" class="store-btn">選擇 7-11 門市</button>
...
export default {
  ...
  watch: {
    address(newValue) {
      this.$emit(
        "update:modelValue",
        `${this.selectedCounty} ${this.selectedDistrict} ${newValue}`
      );
    },
  },
  mounted() {
  ...
  methods: {
    ...
    updateZipcode() {
      this.selectedZipcode =
        this.taiwanAddress[this.selectedCounty]?.[this.selectedDistrict] || "";
    },
  },
