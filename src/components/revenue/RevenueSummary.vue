<template>
    <div class="mb-4">
      <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
          ...
          <select v-model="selectedYear">
            <option disabled v-if="availableYears.length === 0">尚無年份資料</option>
      ...
      <Bar v-else :data="chartData" :options="chartOptions" class="mt-3" height="300" />
  ...
  const availableYears = computed(() => {
    const years = orders.value.map(o => new Date(o.createdAt).getFullYear())
  ...
  const filteredOrders = computed(() => {
    return orders.value.filter(o => {
      ...
      return (
        ...
        (selectedMonth.value === '' || date.getMonth() + 1 === Number(selectedMonth.value))
      )
    })
  })
  ...
  const summaryData = computed(() => {
    ...
    return Array.from(map.entries())
      .sort()
      .map(([key, val]) => ({ key, label: key, count: val.count, revenue: val.revenue }))
  })
  ...
  const exportCSV = () => {
    const header = `${viewMode.value === 'monthly' ? 'Month' : 'Date'},Order Count,Revenue(NT$)`
