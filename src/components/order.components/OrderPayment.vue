        ...
        <div v-else class="payment-content">
            ...
            <div class="order-summary-card">
                ...
                <div class="card-body">
                    <div class="order-info">
                        ...
                        <p><strong>總金額:</strong> <span class="price">NT${{ formatPrice(order.totalPrice) }}</span></p>
                    </div>
                    ...
                    <div class="order-items">
                        ...
                        <div class="items-list">
                            <div v-for="item in order.items" :key="item.sku_id" class="item-row">
                                ...
                                <div class="item-price">NT${{ formatPrice(item.unitPrice) }}</div>
                                <div class="item-total">NT${{ formatPrice(item.unitPrice * item.quantity) }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="shipping-info">
                        <h4>收貨資訊</h4>
                        <div class="info-box">
                            <p><strong>收件人:</strong> {{ order.shipping?.recipientName || '無資料' }}</p>
                            <p><strong>聯絡電話:</strong> {{ order.shipping?.recipientPhone || '無資料' }}</p>
                            <p><strong>收貨地址:</strong> {{ order.shipping?.address || '無資料' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="price-summary">
                        ...
                        <div class="summary-table">
                            <div class="summary-row">
                                <span>商品小計:</span>
                                <span>NT${{ formatPrice(order.subtotal || order.totalPrice) }}</span>
                            </div>
            ...
            <div v-if="selectedPaymentMethod === 'CREDIT_CARD'" class="credit-card-form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="cardNumber" v-model="cardInfo.cardNumber" placeholder="請輸入16位卡號"
                        maxlength="19" @input="formatCardNumber">
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="cardExpiry">有效期限 (MM/YY)</label>
                        <input type="text" id="cardExpiry" v-model="cardInfo.cardExpiry" placeholder="MM/YY"
                            maxlength="5" @input="formatCardExpiry">
                    </div>
                    <div class="form-group">
...
const formatCardExpiry = () => {
    ...
    if (value.length > 2) {
        cardInfo.value.cardExpiry = value.substring(0, 2) + '/' + value.substring(2);
    } else {
    ...
    try {
        ...
        try {
            ...
            if (typeof checkResponse.data === 'string') {
                console.error('收到字符串響應而非JSON:', checkResponse.data);
                error.value = '服務器返回了無效的回應格式';
                loading.value = false;
                return;
            }
            ...
            if (checkResponse.data.status === 'error') {
                error.value = checkResponse.data.message || '此訂單不需要付款或已付款完成';
                loading.value = false;
                return;
            }
        } catch (checkErr) {
        ...
        if (response.data) {
            ...
            if (order.value.orderItem && Array.isArray(order.value.orderItem) && order.value.orderItem.length > 0) {
                order.value.items = order.value.orderItem.map(item => {
                    return {
                        ...
                        // 嘗試構建規格文本
                        specs: item.sku ? `${item.sku.colorName || ''} ${item.sku.sizeName || ''}`.trim() : (item.specs || "")
                    };
                });
            ...
            order.value.subtotal = parseFloat(order.value.subtotal) || order.value.totalPrice;

            // 檢查訂單狀態，確保只有待付款的訂單可以付款
            const orderStatus = order.value.status ||
                (order.value.orderStatusCorrespond?.name || "");

            if (orderStatus !== 'PENDING' &&
                orderStatus !== '待付款' &&
                orderStatus !== 'pending') {
                error.value = '此訂單不需要付款或已經付款完成';
                console.log(`訂單狀態 "${orderStatus}" 不是待付款狀態`);
            }
        } else {
...
const processPayment = async () => {
    ...
    if (selectedPaymentMethod.value === 'CREDIT_CARD') {
        if (!cardInfo.value.cardNumber || cardInfo.value.cardNumber.replace(/\s/g, '').length !== 16) {
            alert('請輸入有效的信用卡號碼');
            return;
        }

        if (!cardInfo.value.cardExpiry || !cardInfo.value.cardExpiry.includes('/')) {
            alert('請輸入有效的到期日期');
            return;
        }
    ...
    try {
        ...
        alert('付款處理失敗: ' + (err.response?.data?.message || err.message || '未知錯誤'));
