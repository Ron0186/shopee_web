<template>
    <div class="container mt-5">
      ...
      <div class="mb-3">
        <button class="btn btn-primary me-2" @click="showPending = true; showRejected = false">待審核</button>
        <button class="btn btn-secondary" @click="showPending = false; showRejected = true">已拒絕</button>
      </div>
        ...
        <tbody>
          <template v-if="applications.length > 0">
            <tr v-for="app in applications" :key="app.applicationId">
              <td>{{ app.applicationId }}</td> <td>{{ app.userId }}</td><td>{{ app.userName }}</td><td>{{ app.shopName }}</td><td>{{ app.shopCategory }}</td><td>{{ app.description }}</td><td>{{ new Date(app.createdAt).toLocaleString() }}</td>
              <td>
                <button class="btn btn-success btn-sm me-2" @click="approveApplication(app.applicationId)">通過</button>
                <button class="btn btn-danger btn-sm" @click="rejectApplication(app.applicationId)">拒絕</button>
              </td>
            </tr>
          </template>
          <tr v-else>
      ...
      <table class="table table-bordered" v-if="showRejected">
        ...
        <tbody>
          <template v-if="rejectedApplications.length > 0">
            <tr v-for="app in rejectedApplications" :key="app.applicationId">
              <td>{{ app.applicationId }}</td><td>{{ app.userId }}</td><td>{{ app.userName }}</td><td>{{ app.shopName }}</td><td>{{ app.shopCategory }}</td><td>{{ app.description }}</td>
              <td>{{ new Date(app.createdAt).toLocaleString() }}</td>
              <td>{{ app.adminComment }}</td>
              <td>{{ app.reviewerName }}</td>
              <td>{{ app.reviewedAt ? new Date(app.reviewedAt).toLocaleString() : '' }}</td>
              <td>
                <button class="btn btn-success btn-sm" @click="approveApplication(app.applicationId)">通過</button>
              </td>
            </tr>
          </template>
          <tr v-else>
        ...
        try {
         ...
         async approveApplication(applicationId) {
          ...
          const { value: confirmResult } = await Swal.fire({
              ...
              cancelButtonText: '取消' //新增取消按鈕
          });
  
          if (confirmResult) {
              try {
                  const response = await axios.post(`/api/shop/application/approve/${applicationId}?adminId=${this.adminId}`);
                  if (response.data.success) {
                      ...
                      this.fetchRejectedApplications(); //重新整理
                  }
                  else {
                      Swal.fire(
                          '錯誤!',
        ...
        if (text) {
          ...
          if (confirmResult) {
            try {
              ...
              const response = await axios.post(`/api/shop/application/reject/${applicationId}?adminId=${this.adminId}&comment=${encodeURIComponent(text)}`);
