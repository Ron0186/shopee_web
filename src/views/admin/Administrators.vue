<template>
  <div class="container mt-4">
      ...
      <tbody>
        <tr v-if="admins.length > 0" v-for="admin in admins" :key="admin.userId">
          ...
          <td>
            ...
            <<<<<<< HEAD <button class="btn btn-danger btn-sm ms-2" @click="deleteUser(admin.userId)">刪除用戶</button>
              =======
    ...
    <admin-profile-edit-modal :isOpen="showProfileModal" :admin="selectedAdmin" @close="showProfileModal = false"
      @save="updateAdminInfo" />

    <admin-add-modal :isOpen="showAddModal" :roles="roleNames" @close="showAddModal = false" @refresh="fetchAdmins" />
  </div>
</template>
