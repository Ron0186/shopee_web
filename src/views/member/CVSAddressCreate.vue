<template>
  <div class="container">
    ...
    <form @submit.prevent="createCVSAddress">
      ...
      <!-- 收件人與手機 -->
      <div>收件人：<input v-model="form.recipientName" required /></div>
      ...
      <button type="submit" :disabled="!selectedStore" class="func">
        儲存
      </button>
      <button type="button" @click="cancel" class="func">取消</button>
      <!-- ← 新增的按鈕 -->
    </form>
...
const selectedStore = computed(() =>
  storeList.value.find((s) => s.StoreId === selectedStoreId.value)
);
...
function validatePhone() {
  const regex = /^09\d{8}$/;
  phoneError.value = regex.test(form.value.recipientPhone)
    ? ""
    : "手機號碼需以09開頭且共10碼";
}
...
async function createCVSAddress() {
  ...
  const fullStreetEtc = `${storeNameMap[cvsType.value]}-${parsed.city}${
    parsed.district
  }${parsed.streetEtc}`;
  ...
  try {
    await axios.post(
      `http://localhost:8081/api/user/address/${userId}/create-cvs`,
      payload,
      {
