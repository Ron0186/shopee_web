      ...
      <table class="campaigns-table">
        ...
        <tbody>
          <tr
            ...
            :class="{ inactive: !isActive(campaign) }"
          >
            ...
            <td class="actions">
              ...
              <button
                class="btn btn-sm btn-coupons"
                @click="openCouponManagementModal(campaign)"
              >
                🎟️ 優惠券
              </button>
              <button
      ...
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="submitCampaignForm">
          ...
          <div class="form-group">
            ...
            <small v-if="imageError" class="text-danger">{{
              imageError
            }}</small>
          </div>
            ...
            <tbody>
              <tr v-for="coupon in campaignCoupons" :key="coupon.couponId">
                ...
                <td class="actions">
                  <button
                    class="btn btn-sm btn-delete"
                    @click="removeCouponFromCampaign(coupon)"
                  >
                    移除
                  </button>
                </td>
...
const handleFileChange = async (event) => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.log(
        "Upload complete - campaignForm.bannerImage:",
        campaignForm.value.bannerImage
      );
    } else {
...
const filterCoupons = () => {
  if (!couponSearchText.value.trim()) {
    ...
    filteredCoupons.value = availableCoupons.value.filter(
      (coupon) =>
...
const submitCampaignForm = async () => {
  try {
    // Check dates
    if (
      new Date(campaignForm.value.endDate) <=
      new Date(campaignForm.value.startDate)
    ) {
      Swal.fire({
        title: "錯誤",
        text: "結束日期必須晚於開始日期",
    ...
    if (editMode.value) {
      ...
      response = await axios.put(
        `/api/campaigns/${formData.campaignId}`,
        formData
      );
    } else {
      // Create new campaign
...
const fetchAvailableCoupons = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      // 過濾掉已經綁定到此活動的優惠券
      const alreadyBoundCouponIds = campaignCoupons.value.map(
        (c) => c.couponId
      );
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.delete(
          `/api/campaigns/${selectedCampaign.value.campaignId}/coupons/${coupon.couponId}`
        );
...
const quickFillForm = () => {
  // 設定預先定義的值
  campaignForm.value.campaignName = "新店開幕";
  campaignForm.value.description = "大大大優惠";

  // 不修改圖片 - 由用戶自行選擇
  // 不修改日期 - 保持原有的預設值或用戶選擇
  // 不修改活動狀態 - 保持預設的 ACTIVE

  // 可選：顯示一個小提示消息
  Swal.fire({
    title: "已自動填入",
    text: "活動名稱和描述已自動填入，請繼續選擇圖片和日期",
    icon: "success",
    timer: 2000,
...
const isActive = (campaign) => {
  ...
  return campaign.status === "ACTIVE" && startDate <= now && endDate >= now;
};
